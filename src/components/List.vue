    ...
    <ol >
      <li class="card" 
        ...
        :key="item.id">
          ...
          <p v-show="item.showMore">Подьезд:{{item.house==0 ? "нет информации" : getInput(item)}}</p>
          <p v-show="item.showMore">Этаж:нет информации</p>
          <p v-show="item.showMore">Корпус:нет информации</p>

          <button @click="triggerShow(item)" class="show">{{!item.showMore ? "Подробнее" : "Скрыть" }}</button>
      </li>
    </ol>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    filterByHouse(){
      if(this.id!==""){
        return this.persons.filter(person=> String(person.house).indexOf(this.id)!=-1)
      }
